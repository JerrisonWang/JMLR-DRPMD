...
class CartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    def add_uncertainty(self, feature_sa, Theta_tran):
        mean = np.dot(Theta_tran, feature_sa)
        
        cov = [[self.cov_val, 0., 0., 0.],
               [0., self.cov_val, 0., 0.], [0., 0.,self.cov_val, 0.], [0., 0., 0., self.cov_val]]
        s_prime = np.random.multivariate_normal(mean, cov)
        return s_prime.tolist()
    
    def set_theta_inner(self,theta_matrix):
        self.theta_matrix = theta_matrix
    def set_cov(self,cov_val):
        self.cov_val = cov_val
    def step(self, action):
        ...
        theta_inner_matrix_trans = self.theta_matrix.T
        self.state = self.add_uncertainty(np.array([x, x_dot, theta, theta_dot]), theta_inner_matrix_trans)
